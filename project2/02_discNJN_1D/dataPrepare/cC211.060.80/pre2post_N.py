...
with h5py.File('/p/arch1/hch02/hch02b/cC211.60.80/0008_r1/twop_ts22_246src.h5') as f:
        ...
        if (not os.path.isfile(outfile)) or os.path.isfile(outfile_flag):
            ...
            with h5py.File(outfile, 'w') as fw, h5py.File(f'{inpath}{file}') as fr, h5py.File(f'{outpathFullmom}{file}','w') as fw2:
                ...
                for src in fr[cfg_old].keys():
                    ...
                    for ky in ['nucl1','nucl2']:
                        ky_new={'nucl1':'N1_N1','nucl2':'N2_N2'}[ky]
                        tF=fr[cfg_old][src][ky]
                        t=tF[...,0]+1j*tF[...,1]
                        if t.shape[1]==3071:
                            t=t[:,momMap_2to1]
                        datDic[ky_new]+=t
