'''
cat data_aux/dat_ignore/analysis_B20_disc_run | xargs -I @ -P 10 python3 -u analysis_B20_disc.py -t @ > log/analysis_B20_disc.out & 
'''
import util as yu
from util import *
...
def run(task):
    ...
    for ens in enss:
        ...
        def createDic(ind,case):
            ...
            tfmins_2st_sum=get_tfs(0.5,0.8)
            tcmins_2st_sum=cutExtraDiff2tcmins(0.6,0.4)
            # WAMA & display
            tf_max=gett(1.5)
            tfmin_max=gett(1.2); tcmin_max=gett(0.6)
            
            tf2ratio=case2tf2ratio[case]
            tf2ratio=yu.cut_tf2ratio(tf2ratio,gett(1.5))
            ...
            fit_const_MA=yu.doMA_3pt(fits_const,tfmin_min=gett(0.9),tfmin_max=tfmin_max,tcmin_min=gett(0.2)*2,tcmin_max=tcmin_max)
            ...
            fit_sum_MA=yu.doMA_3pt(fits_sum,tfmin_max=tfmin_max,tcmin_min=gett(0.2)*2,tcmin_max=tcmin_max)
            ...
            dic={
                ...
                'rainbow:[tfmin,tfmax,tcmin,dt]':[None,gett(1),2,2],
        ...
        def createDic2(ind,case):
            ...
            tcmins_1st=cutExtraDiff2tcmins(1,0.4)
            tfmins_2st_sum=get_tfs(0.4,1.0)
            tcmins_2st_sum=cutExtraDiff2tcmins(0.6,0.4)
            # WAMA & display
            tf_max=gett(1.5)
            tfmin_max=gett(1.2); tcmin_max=gett(0.6)
            ...
            tf2ratio=case2tf2ratio[case]
            tf2ratio=yu.cut_tf2ratio(tf2ratio,gett(1.6))
            
            fits_band=yu.doFits_3pt_band(tf2ratio,tcmins_1st,corrQ=False,unicutQ=True,label=f'{n2qpp1}_{ff}_{j}_{ens}_{case}_band',overwrite=overwrite)
            fit_band_WA=yu.doWA_band(fits_band,tf_min=gett(0.8),tf_max=tf_max,tcmin=gett(0.2)*2,corrQ=False)
            fits_const=yu.doFits_3pt('const',tf2ratio,tfmins_1st,tcmins_1st,unicutQ=True,label=f'{n2qpp1}_{ff}_{j}_{ens}_{case}_const',overwrite=overwrite)
            fit_const_MA=yu.doMA_3pt(fits_const,tfmin_min=gett(0.55),tfmin_max=tfmin_max,tcmin_min=gett(0.2)*2,tcmin_max=tcmin_max)
            fits_sum=yu.doFits_3pt('sum',tf2ratio,tfmins_2st_sum,tcmins_2st_sum,unicutQ=2,label=f'{n2qpp1}_{ff}_{j}_{ens}_{case}_sum',overwrite=overwrite)
            fit_sum_MA=yu.doMA_3pt(fits_sum,tfmin_max=tfmin_max,tcmin_min=gett(0.2)*2,tcmin_max=tcmin_max)
