...
basePath=f'/p/project1/ngff/li47/code/projectData/05_moments/{ens}/'
...
def run(cfg):
    ...
    inpath=f'{basePath}data_pre/Nrun1_100_arch/{cfg_old}/'
    ...
    inpath=f'{basePath}data_pre/Nrun2_127_arch/{cfg_old}.tar'
    file='N.h5_Nrun2_127'
    outfile=f'{outpath}{file}'
    outfile_flag=outfile+'_flag'
    if (not os.path.isfile(outfile)) or os.path.isfile(outfile_flag):
        with open(outfile_flag,'w') as f:
            pass
        with h5py.File(outfile, 'w') as fw, h5py.File(f'{outpathFullmom}{file}','w') as fw2, tarfile.open(inpath) as tar:
            fw.create_dataset('notes',data=['time,mom,dirac',f'[time@fwd]=0:{tmax}; [time@bwd]=-{tmax}:-1','[dirac@fwd]=[0,1,4,5]; [dirac@bwd]=[10,11,14,15]'])
            fw.create_dataset('moms',data=moms_N)
    ...
    inpath=f'{basePath}data_pre/Nrun3_84_arch/{cfg_old}.tar'
    file='N.h5_Nrun3_84'
    outfile=f'{outpath}{file}'
    outfile_flag=outfile+'_flag'
    if (not os.path.isfile(outfile)) or os.path.isfile(outfile_flag):
    ...
    inpath=f'{basePath}data_pre/Nrun4_16/{cfg_old}/'
    files=os.listdir(inpath)
    file='N.h5_Nrun4_16'
    outfile=f'{outpath}{file}'
    outfile_flag=outfile+'_flag'
    ...
    inpath=f'{basePath}data_pre/Nrun5_12/{cfg_old}/'
    files=os.listdir(inpath)
    file='N.h5_Nrun5_12'
    outfile=f'{outpath}{file}'
    outfile_flag=outfile+'_flag'
