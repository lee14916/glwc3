...
def run(cfg):
    cfg_old=cfg2old(cfg)
    outpath=f'/capstor/store/cscs/userlab/lp139/lyan/code/projectData/05_moments/{ens}/data_post/{cfg}/'
    os.makedirs(outpath,exist_ok=True)
    with h5py.File(outpath+'jg.h5','w') as fw:
        flag=True
        for stout in stouts:
            inpath=f'{basePath}conf.{cfg_old}/stout_{stout}/'
            if not os.path.exists(inpath):
                continue
            t_jg=[]
            for i_n in inserts_nums:
                file=inpath+f'gLoops_ultralocal_Clv_EMT_{i_n}_{cfg_old}.dat'
