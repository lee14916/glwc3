...
def cfg2out_jPP(cfg):
    path='/project/s1174/lyan/code/scratch/run/NST_f/cA211.53.24/jPP/out/'+cfg+'/'
    return path

def cfg2out_loop(cfg):
    path='/project/s1174/lyan/code/scratch/run/NST_f/cA211.53.24/loop/out/'+cfg+'/'
    return path
    ...
    if (not os.path.isfile(outfile)) or os.path.isfile(outfile_flag):
        ...
        with h5py.File(outfile,'w') as fw:
            ...
            with h5py.File(infileS) as fs, h5py.File(infileL) as fl: 
                ...
                dat['uBuL']=np.mean([fl['data'][seed][id]['up'][:] for seed in fl['data'].keys() for id in fl['data'][seed].keys()],axis=0)
                ...
                dat['uBuS']=np.mean([fs['data'][seed][id]['up'][:] for seed in fs['data'].keys() for id in fs['data'][seed].keys()],axis=0)
