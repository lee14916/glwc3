...
assert(flags['0tensorNJN']) # ignore 0tensor NJN not implemented
    ...
    for diag in diags:
        ...
        for file in files:
            with h5py.File(inpath+file) as fbase:
                ...
                if flags['0tensorNJN'] and base=='NJN':
                    if t_base.shape[-1]==10:
                        t_base=np.concatenate([t_base,np.zeros(list(t_base.shape[:-1])+[6])],axis=-1)
                    else:
                        assert(t_base.shape[-1]==16)
                        t_base=np.concatenate([t_base[...,:10],np.zeros(list(t_base.shape[:-1])+[6])],axis=-1)
