...
postcode='NJNpi-a-Nsrc3*3'
def cfg2out(cfg):
    path='/project/s1174/fpittler/code/projectData/nucleon_sigma_term/cA211'+cfg[0]+'.53.24/NJNpi_GEVP/outputdata_sinkMom/'+cfg[1:]+'/'    
    return path
...
def run(cfg):
    ...
    #                             momMap=[dic[tuple(ele[:6])] if tuple(ele[:6]) in dic else -1 for ele in moms]
    #                             momMap_bw=[dic[tuple(ele[:6])] if tuple(ele[:6]) in dic else -1 for ele in -np.array(moms)]
    ...
    #                         mark=np.array([1/0 if i ==-1 else 0 for i in momMap]) # those parts have not really been done
    #                         mark_bw=np.array([1/0 if i ==-1 else 0 for i in momMap_bw]) # those parts have not really been done
    #                         tfw=tfw+mark[None,None,None,:,None]; tbw=tbw+mark_bw[None,None,None,:,None]
                        ...
                        for src in fr.keys():
                            ...
                            for i,fla in enumerate(flas):
                                for j in range(aux.Npack):
                                    fw.create_dataset('data/'+src_new[j]+'/'+fla,data=tfw[i][j].astype('complex64'))
                                    fw_bw.create_dataset('data/'+src_new[j]+'/'+fla,data=tbw[i][j].astype('complex64'))
            if flags['wrong_W2pt_bw'] and diag=='W':
                os.remove(outfile_bw)
            os.remove(outfile_flag)
