            ...
            with h5py.File(inpath+file) as fbase:
                ...
                if 'pi0f' in apps:
                    ...
                    for fla in flas:
                        if '_deltat_' in fla:
                            ...
                            fla_new='_'.join([a+',pi0',j,b,dt,tf])
                            flas_new.append(fla_new)
                        else:
                            a,b=fla.split('_')
                            fla_new='_'.join([a+',pi0',b])
                            flas_new.append(fla_new)
                    flas=flas_new
                ...
                if 'pi0i' in apps:
                    ...
                    for fla in flas:
                        if '_deltat_' in fla:
                            a,j,b,dt,tf=fla.split('_')
                            fla_new='_'.join([a,j,b+',pi0',dt,tf])
                            flas_new.append(fla_new)
                        else:
                            a,b=fla.split('_')
                            fla_new='_'.join([a,b+',pi0'])
                            flas_new.append(fla_new)
                    flas=flas_new
                    with h5py.File(aux.cfg2post(cfg,'pi0f')) as f:
                        def getApp(st):
                ...
                #===================
                    ...
                    for ind_fla,fla in enumerate(flas):
                        for tf in aux.tfList:
                            for j in js:
                                a,b=fla.split('_')
                                fla_new='_'.join([a,j,b+',pi0','deltat',str(tf)])
                                flas_new.append(fla_new); 
                                flaMap.append(ind_fla); fla2appky[fla_new]=j+'_pi0'
                    flas=flas_new
                ...
                #===================
                    ...
                    for ind_fla,fla in enumerate(flas):
                        for tf in aux.tfList:
                            for j in js:
                                a,b=fla.split('_')
                                fla_new='_'.join([a+',pi0',j,b,'deltat',str(tf)])
                                flas_new.append(fla_new); 
                                flaMap.append(ind_fla); fla2appky[fla_new]=j+'_pi0'
                    flas=flas_new
                    with h5py.File(aux.cfg2post(cfg,'jPi')) as f:
                    ...
                    for ind_fla,fla in enumerate(flas):
                        for tf in aux.tfList:
                            for j in js:
                                for pi in pis:
                                    a,b=fla.split('_')
                                    fla_new='_'.join([a+','+pi,j,b+','+pi,'deltat',str(tf)])
                                    flas_new.append(fla_new); 
                                    flaMap.append(ind_fla); fla2appky[fla_new]='_'.join([pi,j,pi])
                    flas=flas_new
                    with h5py.File(aux.cfg2post(cfg,'PJP')) as f:
                        def getApp(st,fla):
                            t=f['data']['st{:03}'.format(st)][fla2appky[fla]+'_deltat_'+str(fla2tf(fla))][:]
                            timeMap = [(bwSgn*tc)%aux.lat_T for tc in range(aux.Tpack)]
                            return t[timeMap]
                        dic=aux.moms2dic(f['moms'][:])
                        moms=aux.moms_full2base(moms_full,{'pia','j','pib'})
                        momMap=[dic[tuple(mom)] for mom in bwSgn*moms]
                        t=np.array([[getApp(src[-1],fla) for fla in flas] for src in srcs_int])
                        t_app=t[:,:,:,momMap]
                        if isBW:
                            t_app=t_app*aux.sgn_PT[None,None,None,None,:]
                        t_phase=np.array([[get_phase(src,mom[9:12]+mom[6:9]-mom[0:3]) for mom in bwSgn*moms] for src in srcs_int])
                        t_app=t_app*t_phase[:,None,None,:,None]
                        timeMap_base = [fla2tf(fla) for fla in flas]
                        t_base=t_base[:,flaMap,timeMap_base]
                        t_base=t_base[:,:,None,:,:,:]*t_app[:,:,:,:,None,:]  
